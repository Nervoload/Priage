...
export function ChatPanel({ encounter, messages, onSendMessage, hideHeader = false }: ChatPanelProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[linear-gradient(180deg,_rgba(248,250,252,0.9)_0%,_rgba(255,255,255,1)_40%)]">
      {!hideHeader && (
        <div className="border-b border-slate-200 bg-white px-6 py-4 backdrop-blur-sm">
      ...
      <div className={`flex-1 overflow-y-auto px-6 custom-scrollbar ${hideHeader ? 'py-6' : 'py-5'}`}>
        {messages.length === 0 ? (
          <div className="flex h-full items-center justify-center text-center">
            <div className="max-w-xs rounded-lg border border-slate-200 bg-white px-5 py-6 shadow-sm">
            ...
            {messages.map((message) => {
              ...
              return (
                <div key={message.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`
                      max-w-[72%] rounded-lg px-4 py-3 text-sm shadow-sm
                      ${isAdmin
                        ? 'rounded-br-[6px] bg-priage-700 text-white'
                        : 'rounded-bl-[6px] border border-slate-200 bg-white text-slate-800'
                      }
                    `}
                  >
      ...
      <div className="border-t border-slate-200 bg-white px-6 py-4">
        <div className="flex items-end gap-3">
          <textarea
            ...
            rows={1}
            className="
              max-h-24 min-h-[44px] flex-1 resize-none rounded-lg border border-slate-200 bg-white px-4 py-3
          ...
          <button
            ...
            className="
              shrink-0 rounded-lg bg-priage-700 px-4 py-3 text-sm font-semibold text-white
