...
export async function registerPatient(payload: RegisterPayload): Promise<AuthResponse> {
  if (isStaticDemoMode()) {
    trackDemoEvent('demo_patient_register', { hasEmail: Boolean(payload.email) });
    return { patient: getDemoPatientProfile() as PatientProfile };
  }
...
export async function loginPatient(payload: LoginPayload): Promise<AuthResponse> {
  if (isStaticDemoMode()) {
    trackDemoEvent('demo_patient_login', { hasEmail: Boolean(payload.email) });
    return { patient: getDemoPatientProfile() as PatientProfile };
  }
...
export async function getMe(): Promise<PatientProfile> {
  if (isStaticDemoMode()) {
    return getDemoPatientProfile() as PatientProfile;
  }
...
export async function updateProfile(payload: UpdateProfilePayload): Promise<PatientProfile> {
  if (isStaticDemoMode()) {
    return updateDemoPatientProfile(payload) as Promise<PatientProfile>;
  }
...
export async function submitPatientFeedback(payload: SubmitPatientFeedbackPayload): Promise<void> {
  if (isStaticDemoMode()) {
    trackDemoEvent('patient_feedback_submitted', payload as unknown as Record<string, unknown>);
    return;
  }
...
export async function deletePatientAccount(payload: DeletePatientAccountPayload): Promise<void> {
  if (isStaticDemoMode()) {
    trackDemoEvent('patient_account_delete_attempted', { email: payload.email });
    return;
  }
...
export async function logout(): Promise<void> {
  if (isStaticDemoMode()) {
    trackDemoEvent('demo_patient_logout');
    return;
  }
...
export async function upgradeGuestAccount(payload: UpgradeGuestPayload): Promise<AuthResponse> {
  if (isStaticDemoMode()) {
    const patient = await updateDemoPatientProfile(payload);
    trackDemoEvent('demo_patient_guest_upgraded', { hasEmail: Boolean(payload.email) });
    return { patient: patient as PatientProfile };
  }
